...
async function main() {
  for (const item of RoundClaimParams[20]) {
  ...
  const estimate = BigNumber.from(
    await ethers.provider.call({
      ...
      data: cvxLocker.interface.encodeFunctionData("harvestVotium", [RoundClaimParams[20], 0]),
    })
  );
  ...
  const tx = await cvxLocker.harvestVotium(RoundClaimParams[20], estimate.mul(9).div(10), {
