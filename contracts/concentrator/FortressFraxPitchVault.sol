...
// TSADOK
// import "../interfaces/frax/IERC20.sol";
import "../interfaces/frax/IFraxswapPair.sol";
import "../interfaces/frax/IUniswapV2Router.sol";
import "../interfaces/frax/IUniswapV2Factory.sol";

// solhint-disable no-empty-blocks, reason-string
contract FortressFraxPitchVault is OwnableUpgradeable, ReentrancyGuardUpgradeable, IAladdinConvexVault {
  ...
  struct PoolInfo {
    ...
    // uint256 poolBoosterProviderPoolId;

    // TSADOK
    // The pool address (at Curve, Fraxswap, ...).
    address poolAddress;  
    
    // The pool booster address (at Convex, Pitch, ...).
    address poolBoosterAddress; 
    
    // The address of deposited token.
    address lpToken;
    
    // The address of the booster (Convex, Pitch, etc.) reward contract.
    address crvRewards;
  ...
  address private constant CONVEX_CRV_BOOSTER = 0xF403C135812408BFbE8713b5A23a04b3D48AAE31;
  address private constant CONVEX_FXS_BOOSTER = 0xA2cF21b157b2f203e37b616b619f438B5aa86Ee5;
  ...
  function initialize(address _aladdinCRV, address _zap, address _platform) external initializer {
  ...
  /// @notice See {IAladdinConvexVault-deposit}
  function deposit(uint256 _pid, address _recipient, uint256 _amount) public override onlyExistPool(_pid) returns (uint256 share) {
    ...
    address _lpToken = _pool.lpToken;
    {
      ...
      // uint256 _before = IFraxswapPair(_lpToken).balanceOf(address(this));
      // IFraxswapPair(_lpToken).transferFrom(msg.sender, address(this), _amount);
      // _amount = IFraxswapPair(_lpToken).balanceOf(address(this)) - _before;
    }
  ...
  // TSADOK: helper function
  function addPool(
    address _poolAddress, 
    address _poolBoosterAddress, 
    address _lpToken) 
  external onlyOwner {
    addPool(0, _poolAddress, _poolBoosterAddress, _lpToken, new address[](0), 0, 0, 0);
  }

  /// @dev Add new Convex pool.
  /// @param _convexPid - The Convex pool id.
  /// @param _lpToken - The liquidity pool token.
  ...
  // ) external onlyOwner {
    ...
    poolInfo.push(
      PoolInfo({
        ...
        // The pool booster provider id (Convex, Pitch, ...).
        poolBoosterAddress: _poolBoosterAddress, 
        
        // lpToken: _info.lptoken,
        // crvRewards: _info.crvRewards,

        // The address of deposited token (here it's Frax)
        lpToken: _lpToken,
  ...
  function _depositAtConvex(uint256 _pid, uint256 _amount) internal {
    PoolInfo storage _pool = poolInfo[_pid];

    _approve(_pool.lpToken, _pool.poolBoosterAddress, _amount);
    // IConvexBooster(_pool.poolBoosterAddress).deposit(_pool.convexPoolId, _amount, true);

    IConvexBooster(_pool.poolBoosterAddress).deposit(100, _amount, true);

  }

  // function getBoosterBalance(uint256 _pid, uint256 _amount) external returns (uint256) {
  //   PoolInfo storage _pool = poolInfo[_pid];
  //   return _pool.lpToken.balanceOf(_pool.poolBoosterAddress);
    
  // }
  function _deposit(uint256 _pid, address _recipient, uint256 _amount) internal nonReentrant returns (uint256) {
    PoolInfo storage _pool = poolInfo[_pid];

    // _approve(_pool.lpToken, BOOSTER, _amount);
    // IConvexBooster(BOOSTER).deposit(_pool.convexPoolId, _amount, true);

    if (_pool.poolBoosterAddress == CONVEX_CRV_BOOSTER || _pool.poolBoosterAddress == CONVEX_FXS_BOOSTER  ){
      _depositAtConvex(_pid, _amount);
    }
