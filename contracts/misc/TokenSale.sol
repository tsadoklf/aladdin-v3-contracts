...
contract TokenSale is Ownable, ReentrancyGuard {
    ...
    constructor(address _weth, address _quota, address _base, address _zap, uint128 _cap) {
        weth = _weth;
        quota = _quota;
        base = _base;
        zap = _zap;
    ...
    function buy(address _token, uint256 _amountIn, uint256 _minOut) external payable nonReentrant returns (uint256) {
        ...
        require(_totalSold <= type(uint128).max, "TokenSale: overflow");
  ...
  /********************************** Restricted Functions **********************************/
    ...
    function updateSaleTime(uint64 _whitelistSaleTime, uint64 _publicSaleTime, uint64 _publicSaleDuration) external onlyOwner {
    ...
    function updatePrice(uint96 _initialPrice, uint32 _upRatio, uint128 _variation) external onlyOwner {
  ...
  /// @param _duration The vesting duration, in seconds.
    function updateVesting(address _vesting, uint32 _vestRatio, uint64 _duration) external onlyOwner {
