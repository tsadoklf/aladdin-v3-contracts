...
contract AladdinConvexVaultZap is Ownable, IZap {
  ...
  /********************************** Restricted Functions **********************************/
    ...
    function updateRoute(address _fromToken, address _toToken, uint256[] memory route) external onlyOwner {
  ...
  /********************************** Internal Functions **********************************/
    ...
    function _swapUniswapV2Pair(address _pool, uint256 _indexIn, uint256 _indexOut, uint256 _amountIn) internal returns (uint256) {
        uint256 _rIn;
        uint256 _rOut;
        address _tokenIn;
    ...
    function _swapUniswapV3Pool(address _pool, uint256 _indexIn, uint256 _indexOut, uint256 _amountIn) internal returns (uint256) {
        address _tokenIn;
        address _tokenOut;
        ...
        IUniswapV3Router.ExactInputSingleParams memory _params = IUniswapV3Router.ExactInputSingleParams(_tokenIn, _tokenOut, _fee, address(this),
    ...
    function _swapCurve3Pool(address _pool, uint256 _indexIn, uint256 _indexOut, uint256 _amountIn) internal returns (uint256) {
    ...
    function _swapTriCryptoPool(address _pool,uint256 _indexIn,uint256 _indexOut,uint256 _amountIn) internal returns (uint256) {
    ...
    function _swapCurveV2Pool(address _pool, uint256 _indexIn, uint256 _indexOut, uint256 _amountIn) internal returns (uint256) {
    ...
    function _swapCurveFactoryPool(address _pool, uint256 _indexIn, uint256 _indexOut, uint256 _amountIn) internal returns (uint256) {
        ...
        return ICurveFactoryPlainPool(_pool).exchange(int256(_indexIn).toInt128(), int256(_indexOut).toInt128(), _amountIn, 0, address(this));
